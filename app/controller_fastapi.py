...
async def get_psnr_score(file_input: UploadFile = File(...),
    ...
    try:
        ...
        with open(file_refer_path, 'wb') as f:
            f.write(res_refer)
        model_handler = PlayerIndex(video_quality_dict={"input_video": file_input_path, "refer_video": file_refer_path})
        video_quality_result_score = model_handler.get_video_quality_psnr()
        os.remove(file_input_path)  # 删除临时视频文件
        os.remove(file_refer_path)
        if video_quality_result_score['psnr_score'] is not None:
            return {
                "code": 0,
                "message": "Success",
                "data": video_quality_result_score
...
async def get_niqe_score(file_input: UploadFile = File(...)):
    try:
        ...
        video_quality_result_score = model_handler.get_video_quality_niqe()
        os.remove(file_input_path)  # 删除临时视频文件
        if video_quality_result_score['niqe_score'] is not None:
            return {
                "code": 0,
                "message": "Success",
                "data": video_quality_result_score
